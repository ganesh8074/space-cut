...
def form_type1(prefill: Dict=None, button_label: str="Add"):
    ...
    data = dict(
        ...
        back_thick=back_thick, groove=groove,plinth=plinth
    )
...
def calc_type1(d: Dict) -> List[str]:
    ...
    F2 = 4
    F3 = 2.2
    out.append(_fmt("Facia", 1, door_h/F1, door_w, f"{WOOD}mm core"))
    out.append(_fmt("Facia", 1, door_h/F2, door_w, f"{WOOD}mm core"))
    out.append(_fmt("Facia", 1, door_h/F3, door_w, f"{WOOD}mm core"))

    out.append(_fmt("Tandem Back", 1, door_h/F1/2, door_w - P, f"{WOOD}mm core"))
    out.append(_fmt("Tandem Back", 1, door_h/F2/2, door_w - P, f"{WOOD}mm core"))
    out.append(_fmt("Tandem Back", 1, door_h/F3/2, door_w - P, f"{WOOD}mm core"))


    tab_bd = D - 3*WOOD_IN_OUT
    tab_bw = L - P
    out.append(_fmt("Tandem Bottom", 3, tab_bd, tab_bw, f"{WOOD}mm core"))
...
def get_cutlist_df(d: Dict) -> pd.DataFrame:
    ...
    rows.append(_row("Top Back Ripper", 1, tb_len, t_w, f"{WOOD}mm core"))
    ...
    F2 = 4
    F3 = 2.2
    rows.append(_row("Facia", 1, door_h/F1, door_w, f"{WOOD}mm core"))
    rows.append(_row("Facia", 1, door_h/F2, door_w, f"{WOOD}mm core"))
    rows.append(_row("Facia", 1, door_h/F3, door_w, f"{WOOD}mm core"))

    rows.append(_row("Tandem Back", 1, door_h/F1/2, door_w - P, f"{WOOD}mm core"))
    rows.append(_row("Tandem Back", 1, door_h/F2/2, door_w - P, f"{WOOD}mm core"))
    rows.append(_row("Tandem Back", 1, door_h/F3/2, door_w - P, f"{WOOD}mm core"))


    tab_bd = D - P
    tab_bw = L - P
    rows.append(_row("Tandem Bottom", 3, tab_bd, tab_bw, f"{WOOD}mm core"))
