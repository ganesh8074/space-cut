...
def form_type1(prefill: Dict=None, button_label: str="Add"):
    ...
    st.subheader("Kitchen Blind Corner – Inputs")
...
def calc_type1(d: Dict) -> List[str]:
    ...
    door_h = H -2*OUT
    door_w = L/2
    out.append(_fmt("Doors", 1, door_h, door_w, f"{WOOD}mm core"))

    out.append(_fmt("Dummy", 3, H, 70, f"{WOOD}mm core"))
    out.append(_fmt("Dummy", 1, H, L/1.5, f"{WOOD}mm core"))
...
def get_cutlist_df(d: Dict) -> pd.DataFrame:
    ...
    door_h = H -2*OUT
    door_w = L
    rows.append(_row("Doors", 1, door_h, door_w, f"{WOOD}mm core"))
